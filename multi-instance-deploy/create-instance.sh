...
# 8a. startup-services.sh (container entrypoint launcher)
if [ -f "${SCRIPTS_SRC}/startup-services.sh" ]; then
    cp "${SCRIPTS_SRC}/startup-services.sh" "${WORKDIR}/startup-services.sh"
    chmod +x "${WORKDIR}/startup-services.sh"
    echo "  Deployed startup-services.sh"
else
    echo "  WARNING: startup-services.sh template not found, skipping"
fi

# 8b. watchdog.sh (process monitor + auto-restart)
if [ -f "${SCRIPTS_SRC}/watchdog.sh" ]; then
    cp "${SCRIPTS_SRC}/watchdog.sh" "${WORKDIR}/watchdog.sh"
    chmod +x "${WORKDIR}/watchdog.sh"
    echo "  Deployed watchdog.sh"
else
    echo "  WARNING: watchdog.sh template not found, skipping"
fi

# 8c. check-token-sync.py (token health validator)
if [ -f "${SCRIPTS_SRC}/check-token-sync.py" ]; then
    cp "${SCRIPTS_SRC}/check-token-sync.py" "${WORKDIR}/check-token-sync.py"
